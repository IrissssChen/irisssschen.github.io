...
const PUZZLE_GENERATION_VERSION = 13;
...
function linkedChainGroups(list, mode = "standard") {
    ...
    while (groupWords.length < size && sourceCount < targetSourceCount) {
      ...
      const directOptions = [...sourcePool].flatMap(sourceIndex =>
        sourceAlternatives[sourceIndex].flatMap(candidate =>
          groupWords.flatMap((parentWord, parentIndex) => {
            const direction = compatibleDirection(parentWord, candidate);
            return direction ? [{
              sourceIndex,
              parentIndex,
              direction,
      ...
      if (selection) {
        ...
        const bridgeOptions = bridgePool.flatMap((candidate, bridgeIndex) => {
          ...
          return groupWords.flatMap((parentWord, parentIndex) => {
            const direction = compatibleDirection(parentWord, candidate);
            if (!direction) return [];
        ...
        if (!selection && groupWords.length + 2 <= size) {
          ...
          while (queue.length && !bridgePath) {
            ...
            if ([...sourcePool].some(sourceIndex => sourceAlternatives[sourceIndex].some(option =>
              compatibleDirection(currentWord, option)
            ))) {
              bridgePath = state;
              break;
            }
            ...
            bridgePool.forEach((candidate, bridgeIndex) => {
              ...
              const direction = compatibleDirection(currentWord, candidate);
              if (!direction) return;
              queue.push({
  ...
  groups.forEach(group => {
    while (remaining.size && group.words.length < size) {
      const connection = [...remaining].flatMap(sourceIndex =>
        sourceAlternatives[sourceIndex].flatMap(candidate =>
          group.words.flatMap((parentWord, parentIndex) => {
            const direction = compatibleDirection(parentWord, candidate);
  ...
  orphanGroups.forEach(orphan => {
    ...
    // No playable puzzle has spare "disconnected part" capacity left: park
    // the leftover word in a small holding group made entirely of such
    // words (each is its own single-word piece), keeping each holding
    // group within the MAX_DISCONNECTED_PARTS limit too.
...
function linkedHoneycombGroups(list) {
  ...
  return linkedChainGroups(list, "hex");
}
...
function soundUnits(reading) {
  ...
  for (let index = 0; index < chars.length; index += 1) {
    ...
    const vowel =
      aRow.includes(lastKana) || lastKana === "ゃ" || lastKana === "ぁ" ? "a" :
...
function computeAllowedSignatures() {
  ...
  visit();
  return allowed.map((signaturesForPosition, position) =>
...
function renderBoard() {
  ...
  el("puzzle-board").innerHTML = `<svg class="board-lines" aria-hidden="true">
    ...
    const mobileRow = betaPositions ? 8 - desktopColumn : 10 - desktopColumn;
    const mobileColumn = 8 - desktopRow;
...
function renderMapPreview(group) {
  const positions = betaMapMode === "hex"
    ? packedHexPositions(group.words.length)
...
function packedHexPositions(count) {
  return hexVertexPositions.slice(0, count);
}
